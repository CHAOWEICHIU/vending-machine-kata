'use strict'
module.exports = function({quarters = 0, dimes = 0, nickels = 0}){
  const _ = require('lodash')

  let machineContainsCoins = {
				quarters,
				dimes,
				nickels
			}
    , coinsSafeLevel = _.values(machineContainsCoins).filter(num=>num >= 5).length === 3 ? true : false
    , message = coinsSafeLevel ? 'INSERT COIN' : 'EXACT COINT ONLY'
    , acceptedCoins = {
        quarters: 0, // 25
        dimes   : 0, // 10
        nickels : 0 // 5
